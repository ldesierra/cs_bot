...
class HourlyJob
    ...
    while not_finished &&
      ...
      if response.code == 200
        if response["data"].empty?
          ...
          if low_floats.any?
            messages << "found: #{low_floats.map { |item| "#{item["market_name"]} - #{item["id"]} with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
          if good_keychain.any?
            messages << "found: #{good_keychain.map { |item| "#{item["market_name"]} - #{item["id"]} with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
          if katowice_2014_items.any?
            messages << "found: #{katowice_2014_items.map { |item| "#{item["market_name"]} - #{item["id"]} with stickers #{ item["stickers"]&.pluck("name") } with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
          if nice_fade_items.any?
            messages << "found: #{nice_fade_items.map { |item| "#{item["market_name"]} - #{item["id"]} with fade percentage #{ item["fade_percentage"] } and above price #{ item["above_recommended_price"] } with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
          if blue_gem_items.any?
            messages << "found: #{blue_gem_items.map { |item| "#{item["market_name"]} - #{item["id"]} with blue percentage #{ item["blue_percentage"] } and above price #{ item["above_recommended_price"] } with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
          if nice_gloves_items.any?
            messages << "found: #{nice_gloves_items.map { |item| "#{item["market_name"]} - #{item["id"]} with above price #{ item["above_recommended_price"] } with price #{ item["purchase_price"].to_f / 162.8 }" }.join(', ')}"
          end
        end
      end
  ...
  def get_good_keychain(response)
    names = ["Hot Howl", "Hot Wurst", "Baby Karat T", "Baby Karat CT", "Diamond Dog", "Semi-Precious", "Lil' Monster", "Titeenium AWP", "Diner Dog", "Die-Cast AK"]

    response["data"].filter { |item| !item["market_name"]&.include?("Charm") }
                    .filter { |item| names.any? { |name| item["keychains"].present? && item["keychains"].first["name"]&.include?(name) } }
                    .filter { |item| (item["purchase_price"] - item["suggested_price"]).to_f / 162.8 < 10 }
  end
  ...
  def get_nice_gloves_items(response)
    ...
    items = response["data"].filter { |item| names.any? { |name| item["market_name"].include?(name) } }
                            ...
                            .filter { |item| item["id"].to_s.starts_with?(ENV["start_with"].to_s) }
