  ...
  def actual_call
    begin
      ...
      if response.code == 200
        ...
        elsif nice_fade_items.any? && (Time.now.hour < 22 || Time.now.hour >= 10)
          nice_fade_items.each do |item|
            BidJobFade.perform_async(item, false)
          end
        elsif nice_gloves_items.any? && (Time.now.hour < 22 || Time.now.hour >= 10)
          nice_gloves_items.each do |item|
            BidJobGloves.perform_async(item)
          end
        elsif m4_and_awp_fade.any? && (Time.now.hour < 22 || Time.now.hour >= 10)
          m4_and_awp_fade.each do |item|
            BidJobFade.perform_async(item, true)
          end
        elsif low_floats.present? && low_floats.any? && (Time.now.hour < 22 || Time.now.hour >= 10)
